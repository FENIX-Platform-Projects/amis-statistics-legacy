...
OpenLayers.Control.OWSManager.prototype = 
  ...
  // DOM Elements
    ...
    */  
        ...
        for( var i = 0; i < layers.length; i++) {
            ...
            if (baseLayer || layer.displayInLayerSwitcher) {
				...
				for(j=0;j<aLayersName.length;j++){
					...
					if(layer.OWSName){
						var OWSName = layer.OWSName.split('||||');
						var OWSTitle = layer.OWSTitle.split('||||');
						var OWSAbstract = layer.OWSAbstract.split('||||');
						var OWSBoundingBox = (layer.OWSBoundingBox.indexOf('||||'))?layer.OWSBoundingBox.split('||||'):layer.OWSBoundingBox;
						var OWSLegendURL = layer.OWSLegendURL.split('||||');
						var OWSqueryable = layer.OWSqueryable.split('||||');
						for(y=0;y<OWSName.length;y++){
							var legendUrl = null;
							//var listedLayer = OWSName[y];
							if(selectedLayer==OWSName[y]) {
								//list of layer titles
								var li = document.createElement('li');
								li.innerHTML =  '[' + OWSName[y] + '] ' + OWSTitle[y];
								if(OWSqueryable[y]==1) li.innerHTML +=' (q)'; 
								layerUl.appendChild(li);
								
								 
								//find layers extents
								if(typeof OWSBoundingBox == 'object'){
									aExtents.push(OWSBoundingBox);//bounding boxes
								}else  if(OWSBoundingBox[y].length==4){
									aExtents.push(OWSBoundingBox);//bounding boxes
								} else if(OWSBoundingBox[y].split){
									var bbox = OWSBoundingBox[y].split(',');
									aExtents.push(bbox);//bounding boxes
								} else {
									
									alert('wrong bbox: '+OWSBoundingBox[y]);
								}
							}
						}
					} else {
				...
				//KML Button
				var aKMLlink = document.createElement("a");
				aKMLlink.innerHTML = '<span> KML </span>';
				aKMLlink.title = 'get the KML of this layer to see it in Google Earth';
				aKMLlink.className = 'olLegendIconZoomer';
				//wms/kml_reflect?layers=sf:roads
				//http://test.ominiverdi.org:8080/geoserverSld/wms?
				var href= layer.url;
				var showKML =(layer.url.indexOf('geoserver')=='-1')?null:true;
				if(href && href.charAt( href.length - 1 ) == '?'){
			            href = href.slice( 0, -1 );
					}
				//href = href + '';
				aKMLlink.href = href + '/kml_reflect?layers=' + layer.name;
				aKMLlink.layer = layer;
				aKMLlink.bounds = bounds;
				aKMLlink.onclick = function(){
									//alert(this.href);
				...
				for(j=0;j<aLayersName.length;j++){
					...
					if(layer.OWSName){
						...
						var OWSLegendURL = layer.OWSLegendURL.split('||||');
						var OWSqueryable = layer.OWSqueryable.split('||||');
						var SLDService = layer.SLDService;
						for(y=0;y<OWSName.length;y++){
							var legendUrl = null;
							var listedLayer = OWSName[y];
							if(selectedLayer==OWSName[y]) {
								...
								//set legend url
								legendName = OWSName[y];
								legendTitle = OWSTitle[y];
								legendAbstract = OWSAbstract[y];
								legendQueriable = OWSqueryable[y];
								legendUrl = OWSLegendURL[y];
								...
								if(SLDManager && sldService){
									...
									sldButton.layerName = selectedLayer;
									sldButton.layer = layer;
									sldButton.SLDManager = SLDManager;

									sldButton.sldService = this.sldService;
									sldButton.target = sldman;
									sldButton.legendSpan = legendSpan;
									sldButton.onclick = function(){
															this.SLDManager.updateTarget(this.target);
															this.SLDManager.redraw(this);
															this.SLDManager.activeColor = this.activeColor;
															this.SLDManager.legendSpan = this.legendSpan;
															this.SLDManager.layer = this.layer;
				...
				if(showKML) layerDiv.appendChild(aKMLlink);
    ...
    loadContents: function() {
		...
		this.olCatalogue.style.height = this.map.size.h - 180;
		...
		this.catSubmit.OWSManager = this;
		OpenLayers.Event.observe(this.catSubmit, "click",
                      function(evt) { 
					  	var str = this.OWSManager.catalogueInput.value;
						if(!str) {
							return alert('you can\'t  search an empty string');	
						}

						this.OWSManager.searchCatalogue(this);
					});
		...
		this.olServerList.style.height = this.map.size.h - 180;
		...
		this.serverSelect.OWSManager = this;
        ...
        //
        this.layersDiv.style.width = "98%";
        this.layersDiv.style.height = this.map.size.h - 150;
        ...
        */
		...
		this.olLegend.style.height = this.map.size.h - 180;
    ...
    connect2server: function(evt) {
         this.OWSManager.layerStuff.innerHTML ='';
		 
		 var select = this;
		 if((select)) {
		 	var url = select[select.selectedIndex].value;
			if(select[select.selectedIndex].sldService){
				this.OWSManager.sldService = select[select.selectedIndex].sldService;
			} else {
				this.OWSManager.sldService = '';
			}	 
		...
		$('OWSManagerOutput').innerHTML = "Loading Layers list... please wait...";
        //$('OWSManagerOutput').innerHTML = this.baseURL;
		//alert(this.baseURL);
        OpenLayers.loadURL(this.baseURL, null, this, this.OWSManager.parseGetCapabilities);
    },
    ...
    parseGetCapabilities: function(ajaxRequest) {
		...
		if(typeof xml=='object'){
			...
			if(exceptions.length){
				...
				var exceptionsValue = this.OWSManager.exceptionsValue;
				...
				if(!formatExists) this.OWSManager.exceptionsValue = oFormats[0];
			}
			...
			if(GetMap.length){
				...
				this.OWSManager.aImageFormats = oFormats;
			...
			if(resolutions.length){
				...
				if(resolutionsS.length){
					...
					//this.OWSManager.resolutionsValue = resolutionsS.replace(" ",",");
					this.OWSManager.resolutionsValue = aResolutions;
				} 
			 ...
			 if(aLayer.length>0){
				 //this.OWSManager.drawLayersForm(xml);
				 this.OWSManager.drawLayersForm(xml);
			 } 
    ...
    searchCatalogue: function(obj) {
		var str = obj.OWSManager.catalogueInput.value;

         obj.OWSManager.catalogueLog.innerHTML ='';
	    ...
	    */
		...
		obj.OWSManager.catalogueLog.innerHTML = "Loading Layers list... please wait...";
        //$('OWSManagerOutput').innerHTML = this.baseURL;
		//alert(this.baseURL);
        OpenLayers.loadURL(url, null, obj, obj.OWSManager.parseCatalogueResponse);
    },
    ...
    parseCatalogueResponse: function(ajaxRequest) {
		...
		if(typeof xml=='object'){
			 ...
			 var aLayer = xml.getElementsByTagName('Layer');
			 if(aLayer.length>0){
				 this.OWSManager.drawCatalogueForm(xml);
			 } 
    ...
    drawLayersForm: function(xml) {
		...
		for(i=0;i<aLayers.length;i++){
			...
			this.aWMSLayers.push([Name,Title,Abstract,BoundingBox,LegendURL,queryable,this.sldService]);
    ...
    addWMSLayer: function() {
		...
		for(i=0;i<layerA.length;i++){
			...
			for(j=0;j<this.OWSManager.aWMSLayers.length;j++){
				var checkedLayer = layerA[i];
				var listedLayer = this.OWSManager.aWMSLayers[j];
		...
		//set base or overlay
		var overlayButton = this.OWSManager.overlayButton;
		
		
		//check for base layer
		var layers = this.OWSManager.map.layers.slice();
		...
		//manage exceptions
		if(this.OWSManager.exceptionsValue)
			var exception = this.OWSManager.exceptionsValue;
		 
		 
		 if(this.OWSManager.resolutionsValue){
		 		var aResolutions = this.OWSManager.resolutionsValue;
		...
		*/
		...
		//IMAGE TYPE
		var imageType = this.OWSManager.selectImageType[this.OWSManager.selectImageType.selectedIndex].value;
		...
		if(overlayButton.checked){
			...
			var layer = new OpenLayers.Layer.WMS( layerName, 
                    url, {layers: layerName,format: imageType,TRANSPARENT: "TRUE", EXCEPTIONS: exception} );
			...
			layer.OWSTitle='';
			layer.OWSAbstract='';
			layer.OWSBoundingBox='';
			layer.OWSLegendURL = '';
			layer.OWSqueryable='';
			layer.SLDService='';
			for(aa=0;aa<aWMSLayers.length;aa++){
				var aaWMSLayer = aWMSLayers[aa];
				var separator='';
				...
				//ADD OWS values
					layer.OWSName +=  separator  + aaWMSLayer[0];
					layer.OWSTitle +=  separator  + aaWMSLayer[1];
					layer.OWSAbstract += separator  + aaWMSLayer[2];
					layer.OWSBoundingBox += separator  + aaWMSLayer[3];
					layer.OWSLegendURL += separator  + aaWMSLayer[4];
					layer.OWSqueryable += separator  + aaWMSLayer[5];
					layer.SLDService = aaWMSLayer[6];
			}
			this.OWSManager.map.addLayer(layer);
		} else {
			
			var layer = new OpenLayers.Layer.WMS( layerName, 
                    url, {layers: layerName,format: imageType, EXCEPTIONS: exception} );
			...
			this.OWSManager.map.setBaseLayer(layer, true);
    	    this.OWSManager.map.events.triggerEvent("changebaselayer");
		}
		...
		//var ext = this.OWSManager.extent;
		var bounds = null;
		...
		this.OWSManager.map.zoomToExtent(bounds);
     ...
     *
     */
    addCatalogueLayer: function() {
        var select = this.OWSManager.catalogueSelect;

		if(select) var layer = select[select.selectedIndex];
		else layer ="";
		 ...
		 */
		 ...
		 var Abstract = '';
		 var BoundingBox = [-180,-90,180,90];
		 var HREF = layer.HREF;
		 var url = HREF;
		 var queryable = false;
		 
		 this.OWSManager.aWMSLayers.push([Name,Title,Abstract,BoundingBox,HREF,queryable]);
		 var aWMSLayers = this.OWSManager.aWMSLayers;
		 //Name,Title,Abstract,BoundingBox,HREF,queryable
		...
		if(this.OWSManager.exceptionsValue)
			...
			var exception = ['application%2Fvnd.ogc.se_inimage'];
		 /*
