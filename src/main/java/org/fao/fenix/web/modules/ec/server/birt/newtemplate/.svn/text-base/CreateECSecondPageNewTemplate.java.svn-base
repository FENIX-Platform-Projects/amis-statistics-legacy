...
public class CreateECSecondPageNewTemplate {
	...
	public CreateECSecondPageNewTemplate(ECBean ecBean, ReportDesignHandle designHandle, ElementFactory designFactory ){
...
//		gridCellHandle.setProperty(StyleHandle.PADDING_TOP_PROP, "0.2cm");
	...
	private GridHandle buildFirstColumn() throws SemanticException {
		...
		if ( ecBean.getReportType().equalsIgnoreCase("STANDARD"))
			...
			gridCellHandle.getContent().add(new CreateECSecondPagePakistanAgriculture(ecBean, designHandle, designFactory).buildNaturalDisasters());
