...
public class CreateECSecondPageGovernanceNewTemplate {
	...
	public CreateECSecondPageGovernanceNewTemplate(ECBean ecBean, ReportDesignHandle designHandle, ElementFactory designFactory ){
	...
	private GridHandle buildGovernanceSource() throws SemanticException {
		...
		text.setContent("<div>Different sources</div>");
		gridCellHandle.getContent().add(text);
