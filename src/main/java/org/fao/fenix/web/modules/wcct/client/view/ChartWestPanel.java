...
public class ChartWestPanel {
	private ContentPanel westPanel;
	private VerticalPanel country;
	...
	public ChartWestPanel(){
		westPanel = new ContentPanel();
		country = new VerticalPanel();
		countries = new ListBox();
		apply = new Button("apply");
		options = new VerticalPanel();
		countryMap = new LinkedHashMap<String,String>();
		initializeCountryMap();
		selectedCrop = new String();
	}
	
	private void enhance(WCCTTool wcctTool) {
		countries.addChangeListener(WCCTController.getCropCalendar(wcctTool, countries));
//		apply.addSelectionListener(WCCTController.getCropCalendar(this, wcctTool));
	}
	...
	private VerticalPanel addOptions(){
		...
		options.add(addCountryList());
		return options;
	}

	private VerticalPanel addCountryList() {
		VerticalPanel panel = new VerticalPanel();
		panel.setSpacing(5);
		panel.add(new Html("<b> <font size='2'>Country</font></b>"));
		countries = getCountriesListBox();
		countries.setSize("200px", "25px");	
		panel.add(countries);	
		panel.setHeight(35);
		return panel;
	}
