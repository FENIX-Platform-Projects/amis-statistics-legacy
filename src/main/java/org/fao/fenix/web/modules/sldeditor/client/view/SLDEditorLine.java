...
public class SLDEditorLine extends SLDEditorTabItem {

	private List<SLDEditorRuleForLines> rules;
	private SLDEditorWindow myWindow;
	
	public SLDEditorLine(String header, SLDEditorWindow myWindow) {
		super(header);
		this.myWindow = myWindow;
		this.getTabItem().setSize("385px", "450px");
		//this.getTabItem().setSize("585px", "650px");
		this.getTabItem().setScrollMode(Scroll.AUTO);
		rules = new ArrayList<SLDEditorRuleForLines>();
	}
	
	public TabItem build() {
		if(myWindow.getVoList() == null)
		{
			SLDEditorRuleForLines rule = new SLDEditorRuleForLines(this);
		...
		else
		{
			...
			{
				temp = list.get(iLine);
				if(temp.getType() == SLDEditorConstants.LINE)
				{
					flag=true;
					break;
				}
	...
	public List<SLDEditorRuleForLines> getRules() {
		return rules;
	}
			...
			if(list != null)
			{
				if(list.size() != 0)
				{
					...
					for(int iLine=1; iLine < list.size(); iLine++)
					{
						...
						{
							//System.out.println("SLDEditorConstants. not LINE ");
						}
					}
				...
				int iRules =0;
					for(int iLine=1; iLine < list.size(); iLine++)
					{
						...
						if(temp.getType() == SLDEditorConstants.LINE)
						{
							SLDEditorController.formatLines(rules.get(iRules), this, temp);
							iRules++;
						}
						else
						{
							//System.out.println("SLDEditorConstants. not LINE 2 ");
						}
					}
				}
			}
