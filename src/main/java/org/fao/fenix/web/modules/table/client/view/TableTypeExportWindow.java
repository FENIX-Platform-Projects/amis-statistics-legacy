...
public class TableTypeExportWindow extends FenixWindow {
	
	private ContentPanel contentPanel;
	
	RadioButton code = new RadioButton("selection");
	RadioButton label = new RadioButton("selection");
	RadioButton codelabel = new RadioButton("selection");
	VerticalPanel v;
	
	
	public TableTypeExportWindow(TableWindow tableWindow, long resourceId) {
	...
	private void buildButtonsPanel(TableWindow tableWindow, long resourceId) {
		contentPanel.setButtonAlign(HorizontalAlignment.CENTER);
	
		Button ok = new Button(BabelFish.print().ok());
		ok.setWidth(40);
		ok.addSelectionListener(TableToolbarController.exportToExcel(this, tableWindow, resourceId));
