...
//	}
                    ...
                    if(AMISInputNew.openedCbsTool)
                    {
                        ...
                        {  **/
                            ...
                            public void handleEvent(MessageBoxEvent ce) {
                                ...
                                if((btn!=null)&&(btn.getText().equalsIgnoreCase("YES")))
                                {
                                    AMISInputNew.openedCbsTool = false;
                                    if (RootPanel.get("CLOSE_CBS_TOOL").getWidgetCount() > 0)
                                        RootPanel.get("CLOSE_CBS_TOOL").remove(RootPanel.get("CLOSE_CBS_TOOL").getWidget(0));
                                    setHistoryAnchor(currentView);
                                }
                          ...
                          // MessageBox.confirm("Confirm", "You are changing view. Any modification made in the CBS Input Tool will be lost. Are you sure you want to do this?", l);
                            MessageBox mb = new MessageBox();
                            mb.setIcon(MessageBox.WARNING);
                            mb.setButtons(MessageBox.YESNO);
                            mb.setTitle("Please Confirm");
                            mb.setMinWidth(500);
                            mb.addCallback(l);
                            mb.setMessage("You are changing the section of the site. Are you sure?");
                            mb.setType(MessageBox.MessageBoxType.CONFIRM);
                            mb.show();
                        //}
                    }
                    else
	...
	public static void setHistoryAnchor(final AMISCurrentView currentView) {
        ...
        System.out.println("AMISHistoryController event ::: History.getToken() = '" + currentToken + "' | historyToken = ");
