...
if (window.jQuery === undefined || window.jQuery.fn.jquery !== '1.8.2') {
   ...
   // (document.getElementsByTagName("head")[0] || document.documentElement).appendChild(script_tag);
    	...
    	for (var j=0;j<allScripts.length;j++) { <!-- REVERT THE CODE BACK to http://statistics.amis-outlook.org/data/amis-charts-widget/amis-statistics-charts.js!!-->
...
function main() { 
    ...
    jQuery(document).ready(function($) { 
		...
		domainScript.src = 'http://statistics.amis-outlook.org/data/amis-charts-widget/xdomain/browser.detect.min.js';
	    	  ...
	    	  for (var i=0;i<scripts.length;i++) { <!--REVERT THE CODE BACK to http://statistics.amis-outlook.org/data/amis-charts-widget/amis-statistics-charts.js!!-->
	  ...
	  function getData() {  
	   /******* Create and Load Charts into HTML *******/
	    ...
	    $('#amis_chart_container > table').each(function () { 
	      ...
	      if (typeof productsArray !== 'undefined' && productsArray.length > 0) {
	          // the array is defined and has at least one element
	          try{
		  	      	getDataAndCharts(productsArray);
		  	      	}
		  	      	catch(error){
		...
		// alert("Please check if the id has been set to a product code");
         ...
         function getDataAndCharts(productsArray) {          
	 	     ...
	 	     function next() {
	 	         ...
	 	         if (index < productsArray.length ) {
	 	             var cropcode = productsArray[index];
	 	            // var urlNew = 'http://localhost:8080/fenix-web/data/amis-statistics-jsonp.jsp?&productcode='+cropcode + "&callback=?";
	 	               var urlNew = 'http://statistics.amis-outlook.org/data/amis-charts-widget/jsp/amis-crops-jsonp.jsp?&productcode='+cropcode + "&callback=?";
	 	             ...
	 	             index++;
	 	              ...
	 	              $.ajax({
	 		     	 	 		            ...
	 		     	 	 		            url:        urlNew,
	 		     	 	 		            async: false,
	 		     	 	                             jsonpCallback: 'jsonCallback'+cropcode,
         ...
         function getDataAndChartsOriginal(productsArray) {          
         
          $.each(productsArray, function(i, cropcode) {
